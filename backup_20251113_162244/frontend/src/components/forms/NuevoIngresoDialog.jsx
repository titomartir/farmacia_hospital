...
const NuevoIngresoDialog = ({ open, onClose }) => {
  ...
  const [openNuevoMedicamento, setOpenNuevoMedicamento] = useState(false);
  ...
  const handleNuevoMedicamentoCreado = async (nuevoMedicamento) => {
    // Recargar insumos-presentaciones
    await cargarCatalogos();
    
    // Agregar automáticamente a la tabla de detalles
    setDetalles(prev => [...prev, {
      id_temporal: Date.now(),
      id_insumo_presentacion: nuevoMedicamento.id_insumo_presentacion,
      lote: '',
      fecha_vencimiento: new Date(),
      cantidad: '',
      precio_unitario: ''
  ...
  return (
    <>
              ...
              <Box display="flex" justifyContent="space-between" alignItems="center" mt={2} mb={1}>
                ...
                <Box>
                  <Button
                    ...
                    onClick={() => setOpenNuevoMedicamento(true)}
                    sx={{ mr: 1 }}
                  >
                    Nuevo Medicamento
                  </Button>
      ...
      <NuevoMedicamentoDialog
        open={openNuevoMedicamento}
        onClose={() => setOpenNuevoMedicamento(false)}
        onMedicamentoCreado={handleNuevoMedicamentoCreado}
      />
    </>
  );
};
