...
const Consolidados = () => {
  ...
  const cargarConsolidados = async () => {
    try {
      ...
      console.log('Consolidados cargados:', data);
  ...
  const handleCerrar = async (consolidado) => {
    if (!window.confirm('¿Está seguro de cerrar este consolidado? No podrá modificarlo después.')) {
      return;
    }
  ...
  return (
    <Box>
          ...
          <TableBody>
            {loading ? (
              ...
              consolidados.map((cons) => (
                <TableRow key={cons.id_consolidado}>
                  <TableCell>{cons.id_consolidado}</TableCell>
                  <TableCell>
                    {new Date(cons.fecha_consolidado).toLocaleString('es-GT', {
                      year: 'numeric',
                      month: '2-digit',
                      day: '2-digit',
                      hour: '2-digit',
                  ...
                  <TableCell>{cons.servicio?.nombre_servicio || '-'}</TableCell>
                  <TableCell>
                    <Chip
                      label={cons.turno}
                      size="small"
                      color={cons.turno === 'diurno' ? 'primary' : 'secondary'}
                    />
                  </TableCell>
                  <TableCell>
                    {cons.usuario?.personal?.nombres || ''}{' '}
                    {cons.usuario?.personal?.apellidos || ''}
                  </TableCell>
                  <TableCell align="right">{cons.total_medicamentos || 0}</TableCell>
                  <TableCell align="right">
                    Q{parseFloat(cons.costo_total || 0).toFixed(2)}
                  </TableCell>
                  <TableCell>
                    <Chip
                      label={cons.estado}
                      color={getEstadoColor(cons.estado)}
                      size="small"
                    />
                  </TableCell>
      ...
      {consolidadoSeleccionado && (
        <DetalleConsolidadoDialog
          open={dialogDetalle}
          consolidado={consolidadoSeleccionado}
          onClose={() => {
