...
const EntregarRequisicionDialog = ({ open, requisicion, onClose, onSuccess }) => {
  ...
  const handleDetalleChange = (index, campo, valor) => {
    ...
    // Si cambia el lote, actualizar automáticamente el precio unitario
    if (campo === 'id_lote' && valor) {
      const loteSeleccionado = lotes.find(l => l.id_lote === parseInt(valor));
      if (loteSeleccionado) {
        nuevosDetalles[index]['precio_unitario'] = loteSeleccionado.precio_lote || 0;
      }
    }
